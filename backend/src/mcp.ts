...
function rpcError(id: string | number | null | undefined, code: number, message: string, status = 400, data?: unknown): Response {
  return new Response(JSON.stringify({ jsonrpc: '2.0', id: id ?? null, error: { code, message, ...(data === undefined ? {} : { data }) } }), { status, headers: jsonHeaders() })
}
...
function validateOrigin(req: Request, env: Env): Response | null {
  ...
  return rpcError(undefined, -32001, 'Forbidden origin', 403, { code: 'ORIGIN_NOT_ALLOWED', action_required: 'Add the exact trusted origin to MCP_ALLOWED_ORIGINS in Cloudflare configuration.' })
}
...
function validateProtocolVersion(req: Request, method: string): Response | null {
  ...
  return rpcError(undefined, -32600, `Unsupported MCP protocol version: ${supplied}`, 400, { supported_versions: [...SUPPORTED_PROTOCOL_VERSIONS] })
}
...
function validateAccept(req: Request): Response | null {
  ...
  return acceptsJson && acceptsEvents ? null : rpcError(undefined, -32600, 'MCP POST requests must accept application/json and text/event-stream', 406)
}

function isNotification(message: JsonRpcMessage): boolean { return typeof message.method === 'string' && message.id === undefined }
function isJsonRpcResponse(message: JsonRpcMessage): boolean { return message.method === undefined && (message.result !== undefined || message.error !== undefined) }
...
function methodHeaders(method: string, toolName?: string): HeadersInit {
  return { 'MCP-Protocol-Version': LATEST_PROTOCOL_VERSION, 'MCP-Method': method, ...(toolName ? { 'MCP-Name': toolName } : {}) }
}
function missingGithubTokenResult() {
  return {
    ...
    structuredContent: { ok: false, operation_id: crypto.randomUUID(), status: 'failed', error: { code: 'GITHUB_TOKEN_MISSING', message: 'GitHub access is not configured.', retryable: false, action_required: 'Set GITHUB_TOKEN in Cloudflare Secrets.' } },
    isError: true,
  }
}
...
export async function handleMcp(req: Request, env: Env): Promise<Response> {
  ...
  if (req.method === 'GET' || req.method === 'DELETE') return new Response(null, { status: 405, headers: { Allow: 'POST, GET', 'Cache-Control': 'no-store' } })
  ...
  try {
    ...
    if (Array.isArray(parsed) || !parsed || typeof parsed !== 'object') return rpcError(undefined, -32600, 'A single JSON-RPC object is required')
  ...
  switch (message.method) {
    ...
    case 'tools/list': return rpcResponse(message.id, { tools: ALL_MCP_TOOLS }, methodHeaders('tools/list'))
    case 'tools/call': {
