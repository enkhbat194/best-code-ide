import { legacyGatewayTools } from './toolGateway'
...
function safetyNote(tool: (typeof legacyGatewayTools)[number]): string {
...
function actionPaths(): Record<string, object> {
  ...
  for (const tool of legacyGatewayTools) {
    paths[`/api/actions/${tool.name}`] = {
      post: {
        ...
        responses: {
          '200': { description: 'Structured BestCode tool result. Check ok, status, result, error, safety_class, and audit fields.', content: { 'application/json': { schema: { $ref: '#/components/schemas/ToolEnvelope' } } } },
...
export function openapiSpec(origin: string): object {
  return {
    ...
    info: {
      ...
      description: 'Project-scoped GitHub, Mission, and IDE controller for ChatGPT Actions. MCP and OpenAPI call the same BestCode tool gateway and safety classifier.',
      version: '0.12.0',
    },
    servers: [{ url: origin }],
    security: [{ bearerAuth: [] }],
      ...
      schemas: {
        ...
        ToolEnvelope: {
          ...
          properties: {
            ...
            safety_class: { type: 'string', enum: ['read-only', 'write-without-approval', 'approval-required', 'irreversible'] },
